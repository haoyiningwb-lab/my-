...
- 2026-03-25 起表头已固定：大模型表2字段为 `日期/总进审量/小模型均通过量/大模型进审量/大模型通过量/大模型人审驳回量/大模型命中量/大盘违规率/人工推审率/小模型命中量/数据量误差值/大模型总量占比`；小模型表1字段为 `日期/总进审量/黑样本数/人审总量/大盘违规率/人工推审率/小模型漏放黑样本数`。
- 日报保持历史内容不删；新增日报必须插到最上面。日报更新完成后，立即继续更新业务数据看板。
